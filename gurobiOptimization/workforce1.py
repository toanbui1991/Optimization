# -*- coding: utf-8 -*-
"""
Created on Mon May 22 16:25:40 2017
...
workers, pay = multidict({
  ...
  "Ed":    8,
  "Fred":  9,
  "Gu":    11 }) #with multidict, workers is a list, pay is a dict.
...
('Gu', 'Sun14')
]) #tupleList is just like a list of tuple but it is a gurobi object.
...
#print(x) x is like a dictionary index by (w,s)

# The objective is to minimize the total pay costs
m.setObjective(quicksum(pay[w]*x[w,s] for w,s in availability), GRB.MINIMIZE) #sum over w,and s.

# Constraints: assign exactly shiftRequirements[s] workers to each shift s
reqCts = m.addConstrs((x.sum('*', s) == shiftRequirements[s]
                      for s in shifts), "_")# for each shift s sum over the worker w to find the total number of workers.
...
for c in m.getConstrs(): #get all the constraint
    if c.IISConstr: #get the constraint that make the model infeasible.
