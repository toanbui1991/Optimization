# -*- coding: utf-8 -*-
"""
Created on Wed May 24 00:44:12 2017
...
#Note: you need to produce one product (food) from severl oils
food = model.addVars(time_periods, name="Food")#only one product
...
#Balance
#balance cosntraint for each oil and for each period
#youo have already know how to use if to conditioning.
...
#End Store, for end and start you don't need to consider period
model.addConstrs((store[time_periods[-1], oil] == IStore for oil in oils),"End_Balance")


# Capacity1 & Capacity2
#capacity fo each period and you if to conditioning for two different type of oil.
model.addConstrs(
...
# Hardness
# note the harness constraint is about the quality (hardness) of the final product
model.addConstrs(
...
model.addConstrs(
 ...
 # Conserve
 #note: the amout fo raw material equal to the food product
