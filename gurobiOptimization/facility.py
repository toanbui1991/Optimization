# -*- coding: utf-8 -*-
"""
Created on Sun May 21 05:34:30 2017
...
demand = [15, 18, 14, 20] #4 warehouses

# Plant capacity in thousands of units
capacity = [20, 22, 17, 19, 18] #5plants

# Fixed costs for each plant
fixedCosts = [12000, 15000, 17000, 13000, 16000]

# Transportation costs per thousand units
transCosts = [[4000, 2000, 3000, 2500, 4500], #columns is plants (5), row is warehouses(4)
...
open = m.addVars(plants,
                 vtype=GRB.BINARY,
                 obj=fixedCosts,# cost of opening a plants.
                 name="open") #open the plant or not.
...
m.addConstrs(
    ...
    "Capacity") #plant capacity constraints, sum over the warehouses for each plant.
...
m.addConstrs(
    ...
    "Demand")#demand contraints
#Note: transportation[w,p] but you transportation.sum(w) is equal to transportation.sum(w, '*')
#mean that sum over all plant for each warehouses.

# Guess at the starting point: close the plant with the highest fixed costs;
# open all others

# First, open all plants
#assign the initial solution with open[p].start = 1
for p in plants:
    open[p].start = 1.0 #
